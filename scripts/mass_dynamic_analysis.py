...
Usage example:
python scripts/mass_dynamic_analysis.py \
    -d "data/apk" \
    -s 127.0.0.1:8000 \
    -k YOUR_MOBSF_API_KEY \
    --use-frida \
    --frida-server-path "tools/frida-server/frida-server-15.1.23-android-x86" \
    --frida-script "frida_scripts/my_instrument.js" \
    --adb-path "C:\\path\\to\\adb.exe" \
    --aapt-path "C:\\Android\\Sdk\\build-tools\\33.0.2\\aapt.exe" \
    --wait 60
"""
import argparse
import os
...
def run_cmd(cmd, check=True, shell=False, env=None):
    ...
    if isinstance(cmd, list):
        ...
        proc = subprocess.run(cmd, capture_output=True, text=True, env=env, shell=True)
    out = proc.stdout or ""
    err = proc.stderr or ""
...
def request_dynamic_scan(server: str, api_key: str, scan_endpoint: str, upload_resp: dict, timeout=120) -> Optional[dict]:
...
# ADB / Frida flow
# -----------------------
def adb_push_and_start_frida(adb_path, frida_server_local_path, remote_path="/data/local/tmp/frida-server"):
    print(f"[+] Pushing frida-server to device: {frida_server_local_path} -> {remote_path}")
    ...
    # check
    try:
        out = run_cmd([adb_path, "shell", "ps -A | grep frida || ps | grep frida"], check=False, shell=True)
...
def run_monkey(adb_path, package_name, events=500, throttle_ms=200):
    cmd = [adb_path, "shell", "monkey", "-p", package_name, "--throttle", str(throttle_ms), str(events)]
    print("[+] Running monkey to generate UI events")
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    return p

def frida_instrument_and_run(package_name, frida_script_path, timeout=60):
    if frida is None:
...
def run_dynamic_with_frida(apk_path,
    ...
    frida_instrument_and_run(package, frida_script_path, timeout=run_timeout)

    # 6 run monkey to stimulate UI
    monkey_proc = run_monkey(adb_path, package, events=500, throttle_ms=200)
    try:
        monkey_proc.wait(timeout=run_timeout + 30)
    ...
    print("[+] dynamic artifacts saved to", out_dir)
    return str(out_dir)
...
def parse_args():
    p = argparse.ArgumentParser(description="Mass Dynamic Analysis for MobSF (upload + dynamic scan or frida-run)")
    p.add_argument("-d", "--dir", required=True, help="Directory containing APKs")
    p.add_argument("-s", "--server", required=True, help="MobSF server host:port (e.g. 127.0.0.1:8000)")
    p.add_argument("-k", "--key", required=True, help="MobSF API key")
    p.add_argument("-e", "--dynamic-endpoint", default=DEFAULT_DYNAMIC_ENDPOINT, help="MobSF dynamic scan endpoint path")
    p.add_argument("--wait", type=int, default=30, help="Seconds to wait after triggering dynamic analysis / ADB run")
    p.add_argument("--adb-path", default="adb", help="Optional: path to adb (adb.exe)")
    p.add_argument("--aapt-path", default="aapt", help="Path to aapt (aapt.exe on Windows)")
    p.add_argument("--package-name", default=None, help="Optional: package name to launch when using adb")
    p.add_argument("--dry-run", action="store_true", help="Do not actually post dynamic scan; just upload and print what would be done")
    p.add_argument("--upload-timeout", type=int, default=120, help="Timeout for upload requests")
    p.add_argument("--scan-timeout", type=int, default=120, help="Timeout for dynamic scan requests")
    # frida-specific
    p.add_argument("--use-frida", action="store_true", help="Use frida-server + adb local dynamic analysis instead of MobSF dynamic API")
    p.add_argument("--frida-server-path", default=None, help="Local path to frida-server binary to push to device")
    p.add_argument("--frida-script", default=None, help="Local path to frida JS script to load into target app")
    p.add_argument("--no-tcpdump", action="store_true", help="Disable tcpdump collection on device")
    return p.parse_args()
...
def main():
    ...
    for idx, apk in enumerate(apk_list, start=1):
        ...
        upload_resp = upload_apk(args.server, args.key, apk, timeout=args.upload_timeout)
        ...
        if args.dry_run:
            print("[DRY RUN] Would request dynamic scan with payload derived from upload response.")
            continue
        ...
        if args.use_frida:
            ...
            if not args.frida_server_path or not args.frida_script:
                print("[ERROR] --frida-server-path and --frida-script are required when using --use-frida")
            else:
                try:
                    out_dir = run_dynamic_with_frida(
                        ...
                        use_tcpdump=not args.no_tcpdump
                    )
            ...
            if args.adb_path:
                print("[*] ADB integration requested. Attempting to install & run on emulator/device.")
                try:
                    install_apk(args.adb_path, apk)
                    if args.package_name:
                        # Basic launch via monkey
                        p = run_monkey(args.adb_path, args.package_name, events=100, throttle_ms=200)
                        try:
            ...
            dyn_resp = request_dynamic_scan(args.server, args.key, args.dynamic_endpoint, upload_resp, timeout=args.scan_timeout)
            if dyn_resp:
